...
if (-not ((Test-Path "Dockerfile") -and (Test-Path "docker-compose-update.yml"))) {
...
tar --exclude=".git" --exclude="node_modules" --exclude=".github" --exclude="documentation" -czf $ArchivePath . | Out-Null
...
# Create network first (required for docker-compose.prod.yml)
